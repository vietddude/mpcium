...
func TestDirectRequestSubjects(t *testing.T) {
	assert.Equal(t, "mpc.sdk.req.wallet-1.ecdsa.keygen.session-1", KeygenRequestSubject("wallet-1", rbtypes.ProtocolECDSA, "session-1"))
	assert.Equal(t, "mpc.sdk.req.wallet-1.eddsa.sign.session-2", SignRequestSubject("wallet-1", rbtypes.ProtocolEdDSA, "session-2"))
	assert.Equal(t, "mpc.sdk.req.*.*.keygen.*", KeygenRequestFilterSubject())
	assert.Equal(t, "mpc.sdk.req.*.*.sign.*", SignRequestFilterSubject())
}
...
func TestParseDirectRequestSubject(t *testing.T) {
	target, err := ParseDirectRequestSubject("mpc.sdk.req.wallet-1.eddsa.sign.session-1")
