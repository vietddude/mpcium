...
func TestRuntimeBridgePresenceAndACL(t *testing.T) {
	...
	presenceSub, err := natsConn.SubscribeSync(relayprotocol.PresenceEventSubject("cosigner-1"))
	...
	requireMQTTSubscribe(t, client, relayprotocol.MQTTTopic("cosigner-1", "wallet-1", "#"), 1, func(_ pahomqtt.Client, msg pahomqtt.Message) {
		messages <- msg
	})

	onlineEvent := nextPresenceEvent(t, presenceSub)
	assert.Equal(t, relayprotocol.PresenceStatusOnline, onlineEvent.Status)
	...
	outboundSubject := relayprotocol.OutboundNATSSubject("cosigner-1", "wallet-1", "keygen", "round-1")
	require.NoError(t, natsConn.Publish(outboundSubject, []byte("outbound-payload")))

	select {
	case msg := <-messages:
		assert.Equal(t, relayprotocol.MQTTTopic("cosigner-1", "wallet-1", "keygen", "round-1"), msg.Topic())
		assert.Equal(t, "outbound-payload", string(msg.Payload()))
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for outbound mqtt message")
	}

	inboundSubject := relayprotocol.InboundNATSSubject("cosigner-1", "wallet-1", "keygen", "round-2")
	inboundSub, err := natsConn.SubscribeSync(inboundSubject)
	require.NoError(t, err)

	requireMQTTPublish(t, client, relayprotocol.MQTTTopic("cosigner-1", "wallet-1", "keygen", "round-2"), 1, false, []byte("inbound-payload"))

	natsMsg, err := inboundSub.NextMsg(5 * time.Second)
	require.NoError(t, err)
	assert.Equal(t, "inbound-payload", string(natsMsg.Data))
	assert.Equal(t, "cosigner-1", natsMsg.Header.Get(relayprotocol.HeaderRelayCosignerID))
	assert.Equal(t, "client-a", natsMsg.Header.Get(relayprotocol.HeaderRelayClientID))

	subscribeToken := client.Subscribe(relayprotocol.MQTTTopic("cosigner-2", "wallet-1", "#"), 1, nil)
	require.True(t, subscribeToken.WaitTimeout(5*time.Second))
	require.NoError(t, subscribeToken.Error())
	assert.GreaterOrEqual(t, subscribeToken.(*pahomqtt.SubscribeToken).Result()[relayprotocol.MQTTTopic("cosigner-2", "wallet-1", "#")], byte(0x80))

	forbiddenSub, err := natsConn.SubscribeSync(relayprotocol.InboundNATSSubject("cosigner-2", "wallet-1", "keygen"))
	require.NoError(t, err)
	publishToken := client.Publish(relayprotocol.MQTTTopic("cosigner-2", "wallet-1", "keygen"), 1, false, []byte("forbidden"))
	...
	assert.Equal(t, relayprotocol.PresenceStatusOffline, offlineEvent.Status)
...
func nextPresenceEvent(t *testing.T, sub *nats.Subscription) relayprotocol.PresenceEvent {
	t.Helper()

	msg, err := sub.NextMsg(5 * time.Second)
	require.NoError(t, err)

	var event relayprotocol.PresenceEvent
